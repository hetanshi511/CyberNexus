...
# regardless of what the current working directory is at runtime.
_AGENT_DIR = os.path.dirname(os.path.abspath(__file__))
_TEMPLATES_DIR = os.path.join(_AGENT_DIR, "templates")
...
def add_branding_to_slide(slide, slide_number, total_slides):
    ...
    try:
        # 1. Add Logo
        logo_path = os.path.join(_TEMPLATES_DIR, "logo.png")
...
def visualization_builder_node(state):
    ...
    try:
        ...
        # Fallback if specific template file doesn't exist
        if not os.path.exists(template_path):
            logger.warning(f"Template {template_name} not found. Falling back to light_blue.pptx")
            template_path = os.path.join(_TEMPLATES_DIR, "light_blue.pptx")
        ...
        # Save to a proper temp file (OS temp dir) so it works regardless of CWD
        _, temp_path = tempfile.mkstemp(suffix=".pptx", prefix="autopresenter_")
